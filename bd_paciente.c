...
int carregar_dados_csv(BDPaciente* bd, const char* nome_arquivo_csv) {
    ...
    } else if (bd->quantidade_atual == MAX_PACIENTES) {
...
void consultar_pacientes_por_nome(const BDPaciente* bd, const char* prefixo_nome_original) {
    int encontrados = 0;
    char nome_paciente_minusculo[MAX_NOME];
    char prefixo_nome_minusculo[MAX_NOME];
    ...
    prefixo_nome_minusculo[MAX_NOME - 1] = '\0';
    string_para_minusculas(prefixo_nome_minusculo);
    size_t len_prefixo = strlen(prefixo_nome_minusculo);
    ...
    for (int i = 0; i < bd->quantidade_atual; i++) {
        strncpy(nome_paciente_minusculo, bd->pacientes[i].nome, MAX_NOME - 1);
        nome_paciente_minusculo[MAX_NOME - 1] = '\0';
        string_para_minusculas(nome_paciente_minusculo);
...
void consultar_pacientes_por_cpf(const BDPaciente* bd, const char* prefixo_cpf_original) {
    ...
    prefixo_cpf_processado[MAX_CPF - 1] = '\0';
    string_para_minusculas(prefixo_cpf_processado);
    size_t len_prefixo = strlen(prefixo_cpf_processado);
    ...
    for (int i = 0; i < bd->quantidade_atual; i++) {
        strncpy(cpf_paciente_processado, bd->pacientes[i].cpf, MAX_CPF - 1);
        cpf_paciente_processado[MAX_CPF - 1] = '\0';
        string_para_minusculas(cpf_paciente_processado);
